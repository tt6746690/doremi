...
#             print('pad_to_multiple_of: ', self.pad_to_multiple_of)
...
#             for feature in features:
#                 print('max_label_length: ', max_label_length)
#                 print('len(feature["labels"]): ', len(feature["labels"]))
#                 print('reminder length"): ', (max_label_length - len(feature["labels"])))
...
def get_data_collator(tokenizer, return_tensors='pt', do_padding=False):
    ...
    if do_padding:
        ...
        seq2seq_collator = DataCollatorForSeq2Seq(
            tokenizer=tokenizer, 
            ## wpq: pad to the same length, to avoid error in concatenating batches (from each processes)
    ...
    def data_collator(features):
        if not do_padding:
            ...
            # except padding to longest sequence gives error. for now just pad to `model_max_length`
